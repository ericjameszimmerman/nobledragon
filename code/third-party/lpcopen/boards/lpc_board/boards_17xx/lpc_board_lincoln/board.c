...
void Board_Init(void)
{
    ...
    LPC_IOCON->PINSEL[10] = 0;  // Disable TPIU
...
{
    uint32_t serial[4];
    Chip_IAP_ReadSerial(serial);
    mcaddr[0] = (serial[0] ^ serial[1] ^ (serial[2] >> 8u)) & 0xFC; // XXX 0 and 1 bits are important, see wikipedia and don't spend hours for it again, stupid
    mcaddr[1] = (serial[1] ^ serial[2] ^ (serial[3] >> 16u)) & 0xFF;
    mcaddr[2] = (serial[2] ^ serial[3] ^ (serial[1] >> 24u)) & 0xFF;
    mcaddr[3] = (serial[3] ^ serial[0] ^ (serial[2] >> 8u)) & 0xFF;
    mcaddr[4] = (serial[3] ^ serial[1] ^ (serial[0] >> 16u)) & 0xFF;
    mcaddr[5] = (serial[2] ^ serial[0] ^ (serial[1] >> 24u)) & 0xFF;
}
