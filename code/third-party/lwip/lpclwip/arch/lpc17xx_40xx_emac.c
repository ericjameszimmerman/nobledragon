 ...
 * the same. */
#define tskRECPKT_PRIORITY   (DEFAULT_THREAD_PRIO)
#define tskTXCLEAN_PRIORITY  (DEFAULT_THREAD_PRIO)
#endif
...
uint32_t     g_vPacketReceiveTaskHighWatermark;
...
STATIC void vPacketReceiveTask(void *pvParameters)
{
    ...
    g_vPacketReceiveTaskHighWatermark = uxTaskGetStackHighWaterMark( NULL );
...
uint32_t     g_vTransmitCleanupTaskHighWatermark;
...
STATIC void vTransmitCleanupTask(void *pvParameters)
{
    ...
    g_vTransmitCleanupTaskHighWatermark = uxTaskGetStackHighWaterMark( NULL );
